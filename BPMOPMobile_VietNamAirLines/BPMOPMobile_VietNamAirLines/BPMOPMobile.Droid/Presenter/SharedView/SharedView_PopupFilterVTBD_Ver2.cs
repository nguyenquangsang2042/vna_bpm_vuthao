...
using SQLite;
using static BPMOPMobile.Droid.Core.Controller.ControllerHomePage;
using static BPMOPMobile.Droid.Core.Controller.ControllerHomePage_Ver2;

namespace BPMOPMobile.Droid.Presenter.SharedView
{
    public class SharedView_PopupFilterVTBD_Ver2 : SharedViewBase
    {
        ...
        public ControllerHomePage_Ver2 CTRLHomePage { get; set; }
        ...
        public SharedView_PopupFilterVTBD_Ver2(LayoutInflater _inflater, AppCompatActivity _mainAct, CustomBaseFragment _fragment, string _fragmentTag, View _rootView) : base(_inflater, _mainAct, _fragment, _fragmentTag, _rootView)
        {
        }

        public void InitializeValue(ControllerHomePage_Ver2 CTRLHomePage, int _flagview)
        {
                    ...
                    case (int)FlagViewFilterVTBD.SingleListVTBD:
                        {
                            ...
                            *//*if (_currentFragment._inprocess)
...
*//*
                            if (_currentFragment._inprocess)
                                _lstTrangThai = conn.Query<BeanAppStatus>(String.Format("SELECT ID, Title, TitleEN FROM BeanAppStatus WHERE ID IN ({0})", "1,2,4,128"));
                            else
                                _lstTrangThai = conn.Query<BeanAppStatus>(String.Format("SELECT ID, Title, TitleEN FROM BeanAppStatus WHERE ID IN ({0})", "8, 16, 64"));
                ...
                FragmentSingleListVTBD_Ver2 _currentFrag = null;
                try
                {
                    _currentFrag = (FragmentSingleListVTBD_Ver2)_fragment;
                }
                catch (Exception e)
                {
                ...
                {
                    if (_currentFrag._inprocess)
                    {
                        _lstTrangThai = conn.Query<BeanAppStatus>(String.Format("SELECT ID, Title, TitleEN FROM BeanAppStatus WHERE ID IN ({0})", CmmFunction.GetAppSettingValue(CmmVariable.APPSTATUS_FROMME_DANGXULY)));
                    }
                    else
                    {
                        _lstTrangThai = conn.Query<BeanAppStatus>(String.Format("SELECT ID, Title, TitleEN FROM BeanAppStatus WHERE ID IN ({0})", CmmFunction.GetAppSettingValue(CmmVariable.APPSTATUS_FROMME_DAXULY)));
                    }
                }
                if (_lstTrangThai != null && _lstTrangThai.Count > 0)
                {
                    KeyValuePair<string, string> _KeyVarStatus = new KeyValuePair<string, string>();
                    if (_currentFrag._inprocess)
                    {
                        _KeyVarStatus = CTRLHomePage.LstFilterCondition_VTBD_InProcess.Where(x => x.Key.ToLowerInvariant().Contains("trạng thái")).First();
                    }
                    else
                    {
                        _KeyVarStatus = CTRLHomePage.LstFilterCondition_VTBD_Processed.Where(x => x.Key.ToLowerInvariant().Contains("trạng thái")).First();
                    }
                    ...
                    case (int)FlagViewFilterVTBD.SingleListVTBD:
                        {
                            ...
                            if (_flagDenNgay.Equals(ControllerHomePage.DefaultFilterVDT.EndDate))
                                _flagDenNgay = ControllerHomePage.Hint_EndDate;*/
                            FragmentSingleListVTBD_Ver2 _currentFrag = (FragmentSingleListVTBD_Ver2)_fragment;

                            if (_flagTuNgay.Equals(ControllerHomePage_Ver2.DefaultFilterVDT.StartDate))
                                _flagTuNgay = ControllerHomePage_Ver2.Hint_StartDate;

                            if (_flagDenNgay.Equals(ControllerHomePage_Ver2.DefaultFilterVDT.EndDate))
                                _flagDenNgay = ControllerHomePage_Ver2.Hint_EndDate;
                ...
                _tvApDung.Click += (sender, e) =>
                {
                    ...
                    switch (_flagview)
                    {
                        ...
                        case (int)FlagViewFilterVTBD.SingleListVTBD:
                            {
                                //FragmentSingleListVTBD _currentFragment = (FragmentSingleListVTBD)_fragment;
                                FragmentSingleListVTBD_Ver2 _currentFragment = (FragmentSingleListVTBD_Ver2)_fragment;
                                ...
                                #region Check xem có phải là Default Filter không
                                if (/*_flagTrangThai.Equals(ControllerHomePage.DefaultFilterVTBD.Status) &&*/
                                (_flagTrangThai.Equals(ControllerHomePage_Ver2.DefaultFilterVTBD_Ver2.Status_Processed) ||
                                _flagTrangThai.Equals(ControllerHomePage_Ver2.DefaultFilterVTBD_Ver2.Status_InProcess) ||
                                _flagTrangThai.Equals(ControllerHomePage_Ver2.DefaultFilterVTBD_Ver2.Status)) &&
                                ...
                                var lstTrangthai = _flagTrangThai.Split(",").ToList();

                                _flagTrangThai = String.Join(",", lstTrangthai.Where(x => x != "8" && x != "16" && x != "64").ToArray());
                                CTRLHomePage.LstFilterCondition_VTBD_InProcess = new List<KeyValuePair<string, string>>(){
                                    new KeyValuePair<string, string>("Trạng thái", _flagTrangThai),
                                    new KeyValuePair<string, string>("Hạn xử lý", _flagHanXuLy),
                                    new KeyValuePair<string, string>("Từ ngày", _flagTuNgay),
                                    new KeyValuePair<string, string>("Đến ngày", _flagDenNgay),};
                                _flagTrangThai = String.Join(",", lstTrangthai.Where(x => x != "1" && x != "2" && x != "4" && x != "128").ToArray());
                                CTRLHomePage.LstFilterCondition_VTBD_Processed = new List<KeyValuePair<string, string>>(){
                                    new KeyValuePair<string, string>("Trạng thái", _flagTrangThai),
                                    new KeyValuePair<string, string>("Hạn xử lý", _flagHanXuLy),
                                    new KeyValuePair<string, string>("Từ ngày", _flagTuNgay),
                                    new KeyValuePair<string, string>("Đến ngày", _flagDenNgay),};
                                ...
                                _currentFragment.SetData(); // để trigger lại hàm text changed
                                #endregion
                                break;
                            }
                        case (int)FlagViewFilterVTBD.ChildAppHomePage:
        ...
        private void Click_tvMacDinh(object sender, EventArgs e)
        {
            ...
            {
                // Default Trạng thái
                string[] arrayStatus = ControllerHomePage_Ver2.DefaultFilterVTBD_Ver2.Status.Split(",");
                for (int i = 0; i < _lstTrangThai.Count; i++)
                {
                    if (arrayStatus.Contains(_lstTrangThai[i].ID.ToString()))
                        _lstTrangThai[i].IsSelected = true;
                ...
                for (int i = 0; i < _lstHanXuLy.Count; i++)
                {
                    _lstHanXuLy[i].IsSelected = (_lstHanXuLy[i].ID.Equals(ControllerHomePage_Ver2.DefaultFilterVTBD_Ver2.DueDate)) ? true : false;
                ...
                //_lnNgayDenNgay.SetBackgroundResource(Resource.Color.clTransparent);
                //_lnNgayTuNgay.SetBackgroundResource(Resource.Color.clTransparent);
                _flagTuNgay = ControllerHomePage_Ver2.DefaultFilterVTBD_Ver2.StartDate;
                _flagDenNgay = ControllerHomePage_Ver2.DefaultFilterVTBD_Ver2.EndDate;
