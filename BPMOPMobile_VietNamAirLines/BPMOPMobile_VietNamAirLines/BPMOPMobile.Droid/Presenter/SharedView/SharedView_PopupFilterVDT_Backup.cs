...
{
    public class SharedView_PopupFilterVDT_Backup : SharedViewBase
    {
        ...
        public SharedView_PopupFilterVDT_Backup(LayoutInflater _inflater, Activity _mainAct, CustomBaseFragment _fragment, string _fragmentTag, View _rootView) : base(_inflater, _mainAct, _fragment, _fragmentTag, _rootView)
        {
        }
            ...
            try
            {
                ...
                switch (_flagview)
                {
                    ...
                    case (int)FlagViewFilterVDT.SingleListVDT:
                        {
                            _lnBlurTop.LayoutParameters.Height = ((FragmentSingleListVDT)_fragment)._relaToolbar.Height; // 45dp
                            _popupFilter.Focusable = true;
                            _popupFilter.OutsideTouchable = false;
                            _popupFilter.ShowAsDropDown(((FragmentSingleListVDT)_fragment)._relaToolbar);
                            break;
                        }
                    case (int)FlagViewFilterVDT.ChildAppHomePage:
                        {
                            _lnBlurTop.LayoutParameters.Height = ((FragmentChildAppHomePage)_fragment)._lnToolbar.Height; // 90dp
                            _popupFilter.Focusable = true;
                            _popupFilter.OutsideTouchable = false;
                            _popupFilter.ShowAsDropDown(((FragmentChildAppHomePage)_fragment)._lnToolbar);
                            break;
                        }
                    case (int)FlagViewFilterVDT.ChildAppSingleListVDT:
                        {
                            _lnBlurTop.LayoutParameters.Height = ((FragmentChildAppSingleListVDT)_fragment)._relaToolbar.Height; // 45dp
                            _popupFilter.Focusable = true;
                            _popupFilter.OutsideTouchable = false;
                            _popupFilter.ShowAsDropDown(((FragmentChildAppSingleListVDT)_fragment)._relaToolbar);
                            break;
                        }
                }
                ...
                if (_flagview == (int)FlagViewFilterVDT.SingleListVDT || _flagview == (int)FlagViewFilterVDT.ChildAppSingleListVDT) // view single VDT - đang thực hiện ko cho click
                {
                    FragmentSingleListVDT _currentFrag = (FragmentSingleListVDT)_fragment;
                    if (_currentFrag._flagCurrentTab == (int)ControllerHomePage.FlagStateFilterVDT.InProcess)
                    {
                        _lnTrangThai_Content.Click -= Click_lnTrangThaiContent;
                        ...
                        case (int)FlagViewFilterVDT.SingleListVDT:
                            {
                                ...
                                CTRLHomePage.LstFilterCondition_VDT = new List<KeyValuePair<string, string>>()
                                {
                                    new KeyValuePair<string, string>("Tình trạng",""),
                                    new KeyValuePair<string, string>("Trạng thái",_flagTrangThai),
                                    new KeyValuePair<string, string>("Hạn xử lý", _flagHanXuLy),
                                    new KeyValuePair<string, string>("Từ ngày", _flagTuNgay),
                                    new KeyValuePair<string, string>("Đến ngày", _flagDenNgay),
        ...
        private void Click_imgDenNgayDelete(object sender, EventArgs e)
        {
            try
            {
                ...
                _flagDenNgay = ControllerHomePage.DefaultFilterVDT.StartDate;
