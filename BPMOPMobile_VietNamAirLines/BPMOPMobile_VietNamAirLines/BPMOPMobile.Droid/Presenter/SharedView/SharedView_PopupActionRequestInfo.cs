...
{
    class SharedView_PopupActionRequestInfo : SharedView_PopupActionBase
    {
        ...
        private List<BeanUser> _lstUserQTLC = new List<BeanUser>(); // List user lấy từ QTLC
        public SharedView_PopupActionRequestInfo(LayoutInflater _inflater, AppCompatActivity _mainAct, CustomBaseFragment _fragment, string _fragmentTag, View _rootView, float popupScaleWidth_Full, List<BeanUser> _lstUserQTLC) : base(_inflater, _mainAct, _fragment, _fragmentTag, _rootView)
        {
            this.popupScaleWidth_Full = popupScaleWidth_Full;
            this._lstUserQTLC = _lstUserQTLC;
        ...
        public override void InitializeView()
        {
            ...
            try
            {
                ...
                List<BeanUser> _lstUserAll = _lstUserQTLC.Where(x => x.Email != CmmVariable.SysConfig.Email).ToList(); // bỏ current User ra
                ...
                {
                    List<BeanUser> _lstUserSearch = new List<BeanUser>();
                    if (!String.IsNullOrEmpty(_edtSearchChooseUser.Text)) // Phải lấy những User từ bước trước -> Khác các Action khác
                    {
                        _lstUserSearch = _lstUserAll.Where(x => x.FullName.ToLowerInvariant().Contains(_edtSearchChooseUser.Text.ToLowerInvariant()) || x.Email.ToLowerInvariant().Contains(_edtSearchChooseUser.Text.ToLowerInvariant())).ToList();
                    }
                    else
                    {
                        _lstUserSearch = _lstUserAll.ToList();
                    }

                    if (_lstUserAll != null && _lstUserAll.Count >= 0)
                    {
                        AdapterDetailChooseUser _adapterListUser = new AdapterDetailChooseUser((MainActivity)base._mainAct, _rootView.Context, _lstUserSearch, _selectedUser);
                ...
                _lnChooseUserAction.Click += (sender, e) =>
                {
                    ...
                    var dm = _mainAct.Resources.DisplayMetrics;
                    s.Width = (int)(dm.WidthPixels /** popupScaleWidth_Full*/);
                    s.Height = (int)(dm.HeightPixels /** popupScaleHeight_Full*/);
                    window.Attributes = s;
                };
