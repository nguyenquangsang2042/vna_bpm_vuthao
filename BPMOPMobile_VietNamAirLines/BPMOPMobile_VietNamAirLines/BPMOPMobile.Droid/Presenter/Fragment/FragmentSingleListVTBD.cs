...
{
    public class FragmentSingleListVTBD : CustomBaseFragment
    {
               ...
               if(fromSetData)
                ...
                if (_tempLastVisible == _lstAppBaseVTBD.Count - 1 && _allowLoadMore == true)
                {
                    ...
                    else  // API Filter server
                    {
                        ...
                        {
                            int temp = 0;
                            List<BeanAppBaseExt> _lstMore = _pControlDynamic.GetListFilterMyRequest(CTRLHomePage.GetDictionaryFilter(CTRLHomePage.LstFilterCondition_VTBD, false), ref temp, CmmVariable.M_DataFilterAPILimitData, _lstAppBaseVTBD.Count);
                            _mainAct.RunOnUiThread(() =>
                            {
                                if (_lstMore != null && _lstMore.Count > 0)
                                {
        ...
        public virtual async void SetData()
        {
            ...
            try
            {
                ...
                if (CTRLHomePage.CheckListFilterIsDefault_VTBD(CTRLHomePage.LstFilterCondition_VTBD)) // Default Filter -> offline
                {
                    ...
                    _queryVTBD = CTRLHomePage.GetQueryStringAppBaseVTBD_ByCondition(CTRLHomePage.LstFilterCondition_VTBD, true);
                ...
                else // Filter value -> filter Server
                {
                    ...
                    await Task.Run(() =>
                    {
                        ...
                        int _totalRecord = 0;
                        _lstAppBaseVTBD = pControlDynamic.GetListFilterMyRequest(CTRLHomePage.GetDictionaryFilter(CTRLHomePage.LstFilterCondition_VTBD, false), ref _totalRecord, CmmVariable.M_DataFilterAPILimitData, _lstAppBaseVTBD.Count);
