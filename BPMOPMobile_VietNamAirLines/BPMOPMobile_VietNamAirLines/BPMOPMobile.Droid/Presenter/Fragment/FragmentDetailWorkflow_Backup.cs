...
{
    public class FragmentDetailWorkflow_Backup : CustomBaseFragment
    {
        private float popupScaleWidth_Full = (float)0.9;

        private MainActivity _mainAct;
        private LayoutInflater _inflater;
        private Dialog _dialogPopupControl, _dialogPopupControlGrid;
        private View _rootView;
        private LinearLayout _lnAll, _lnFlowRelated, _lnActionAll, _lnItemAll, _lnData, _lnNoData;
        ...
        public AdapterDetailExpandControl _adapterDetailExpandControl;
        ...
        // Component Comment
        private bool _allowCalculateCommentPosition = true;                                     // Để check xem có cần tính lại vị trí khi scroll -> cho đệ quy khỏi nặng
        private int _commentPosition = 0;
        ...
        #region Constructor
        public FragmentDetailWorkflow_Backup() { /* Prevent Darkmode */ }
        public FragmentDetailWorkflow_Backup(BeanWorkflowItem _workflowItem, BeanNotify _notifyItem, string _previousFragment)
        {
        ...
        public override View OnCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
        {
            ...
            if (_rootView == null)
            {
                ...
                _lnFlowRelated = _rootView.FindViewById<LinearLayout>(Resource.Id.ln_ViewDetailWorkflow_FlowRelated);

                #region View Item
                _lnItemAll = _rootView.FindViewById<LinearLayout>(Resource.Id.ln_ViewDetailWorkflow_All);
                ...
                _expandData.SetFriction(0); // giảm ma sát -> long scroll ko còn trượt
                _expandData.FastScrollEnabled = false;
                ...
                _tvTitle.Text = "";
                _lnItemAll.Visibility = ViewStates.Gone;
            ...
            //_imgComment.Visibility = ViewStates.Gone;
        ...
        private void SetView()
        {
            try
            {
                ...
                else
                {
                    ...
                    _imgSubcribe.SetColorFilter(new Color(ContextCompat.GetColor(_rootView.Context, Resource.Color.clWhite)));
                }
        ...
        private void SetViewItem_ByProcessHistory()
        {
            try
            {
                ...
                else if (_lstQTLC != null & _lstQTLC.Count > 0) // gắn lại avatar người gửi bước trước đó
                {
                    ...
                    if (!String.IsNullOrEmpty(_lstQTLC[0].AssignUserId))
                    {
                        _tvItemDescription.Visibility = ViewStates.Visible;
                        string[] _lstFullName = CTRLHomePage.GetArrayFullNameFromArrayID(_lstQTLC[0].AssignUserId.ToLowerInvariant().Split(","));
                        CTRLHomePage.SetTextView_FormatMultiUser2(_rootView.Context, _tvItemDescription, _lstFullName, IsHighLightColor: true);
                    }
        ...
        private void Click_Back_WithRefreshPage(object sender, EventArgs e)
        {
            try
            {
                switch (_previousFragment)
                {
                    case "FragmentHomePage": // Refresh lại trang home
                        MinionAction.OnRefreshFragmentHomePage(null, null);
                        break;
                    case "FragmentSingleListVDT": // Refresh lại trang FragmentSingleListVDT
                        MinionAction.OnRenewFragmentSingleVDT(null, null);
                        break;
                    case "FragmentSingleListVTBD": // Refresh lại trang FragmentSingleListVTBD
                        MinionAction.OnRenewFragmentSingleVTBD(null, null);
                        break;
                    case "FragmentBoardDetailGroup": // Refresh lại trang FragmentBoardDetailGroup
        ...
        private void Click_tvItemPeople(object sender, EventArgs e)
        {
            try
            {
                ...
                Animation animFadeIn = AnimationUtils.LoadAnimation(_rootView.Context, Resource.Animation.anim_fade_in);
                animFadeIn.Duration = 500; // Custom Duration
                _tvItemDescription.StartAnimation(animFadeIn);

                if (!String.IsNullOrEmpty(_tvItemDescription.Text) && _tvItemDescription.Text.Contains("+")) // Đang ở dạng +n người
                {
                    _tvItemDescription.SetMaxLines(2);
                    string[] _lstFullName = CTRLHomePage.GetArrayFullNameFromArrayID(_workflowItem.AssignedTo.ToLowerInvariant().Split(","));

                    if (CmmVariable.SysConfig.LangCode.Equals(CmmDroidVariable.M_SysLangVN))
                        _tvItemDescription.Text = string.Format("{0}{1}", CmmFunction.GetTitle("TEXT_TO", "Đến: "), String.Join(", ", _lstFullName));
                    else
                        _tvItemDescription.Text = string.Format("{0}{1}", CmmFunction.GetTitle("TEXT_TO", "To: "), String.Join(", ", _lstFullName));
                }
                else
                {
        ...
        private void Click_imgComment(object sender, EventArgs e)
        {
            try
            {
                if (CmmDroidFunction.PreventMultipleClick() == true)
                {
                    ...
                    if (_allowCalculateCommentPosition)
                    {
                        _allowCalculateCommentPosition = false;
                        _commentPosition = 0;
                        for (int i = 0; i < _adapterDetailExpandControl.GroupCount; i++)
                        {
                            _commentPosition += _adapterDetailExpandControl.GetChildrenCount(i) + 1;
                        }
                    }
                    _expandData.SmoothScrollToPosition(_commentPosition);

                    Action action = new Action(() =>
                    {
                        if (_expandData.LastVisiblePosition != _commentPosition - 1)
                        {
                            _allowCalculateCommentPosition = false;
                            Click_imgComment(null, null); // đệ quy lại
                        }
                        else
                        {
                            _allowCalculateCommentPosition = true;
                            _imgComment.StartAnimation(ControllerAnimation.GetAnimationClick_FadeIn(_rootView.Context));
        ...
        private void Click_lnCollapse(object sender, EventArgs e)
        {
            try
            {
                for (int i = 0; i < _LISTSECTION.Count; i++)
                {
                    _LISTSECTION[i].ShowType = false;
            ...
            {
#if DEBUG
                CmmDroidFunction.WriteTrackingError(this.GetType().Name, "Click_LnCollapse", ex);
#endif
            }
        }
        ...
        public async void GetAndSetDataFromServer(bool showdialog = true)
        {
            try
            {
                ...
                await Task.Run(() =>
                {
                    ...
                    {
                        if (_workflowItem.CommentChanged < _CommentChanged)
                        {
                            _workflowItem.IsChange = true;
                        }
                        else if (_CommentChanged == new DateTime()) // Case này để khi API bị lỗi trả ra null thì pass qua -> chỉ dùng để TEST
                        {
                            _workflowItem.IsChange = true;
                        }
        ...
        private void SetData_DynamicControlForm()
        {
            try
            {
                if (_LISTSECTION != null && _LISTSECTION.Count > 0)
                {
                    ...
                    _adapterDetailExpandControl = new AdapterDetailExpandControl(_mainAct, _rootView.Context, _workflowItem, _notifyItem, _LISTSECTION, _LISTFLOWRELATED, _LISTTASK, _LISTCOMMENT, (int)FlagViewControlAttachment.DetailWorkflow, _OtherResourceId);
        ...
        private void SetData_DynamicControlFlowRelated()
        {
            try
            {
                if (_LISTFLOWRELATED != null && _LISTFLOWRELATED.Count > 0)
                {
                    ControlInputWorkRelated controlInputWorkRelated = new ControlInputWorkRelated(_mainAct, _lnFlowRelated, null, -1);
                    controlInputWorkRelated.InitializeFrameView(_lnFlowRelated);
                    controlInputWorkRelated.SetTitle();
                    controlInputWorkRelated.SetValue();
                    controlInputWorkRelated.SetEnable();
                    controlInputWorkRelated.SetProprety();
                }
                else
                {
                    _lnActionAll.Visibility = ViewStates.Gone;
            ...
            {
#if DEBUG
                CmmDroidFunction.WriteTrackingError(this.GetType().Name, "SetData_DynamicControlFlowRelated", ex);
#endif
            }
        }
                ...
                if (CmmDroidFunction.PreventMultipleClick() == true) // không cho click nhiều lần
                {
                    ...
                    switch (_actionID)
                    {
                        case (int)EnumFormControlInnerAction.FlowRelated_InnerActionID.View: // View Full Item -> mở ra trang Detail mới
                            {
                                //SQLite.SQLiteConnection conn = new SQLite.SQLiteConnection(CmmVariable.M_DataPath);
                                //string queryWorkflow = string.Format("SELECT * FROM BeanWorkflowItem WHERE ID = '{0}'", _searchID);
                                //var lstWorkflowItem = conn.Query<BeanWorkflowItem>(queryWorkflow);

                                //string queryNotify = string.Format("SELECT * FROM BeanNotify WHERE SPItemId = '{0}'", _searchID);
                                ...
                                //    {
                                //        FragmentDetailWorkflow detailWorkFlow = new FragmentDetailWorkflow(lstWorkflowItem[0], lstNotify[0], "FragmentDetailWorkflow");
                                //        _mainAct.ShowFragment(FragmentManager, detailWorkFlow, "FragmentDetailWorkflow", 0);
                                //    }
                                //    else
                                //    {
                                //        FragmentDetailWorkflow detailWorkFlow = new FragmentDetailWorkflow(lstWorkflowItem[0], null, "FragmentDetailWorkflow");
                                //        _mainAct.ShowFragment(FragmentManager, detailWorkFlow, "FragmentDetailWorkflow", 0);
                                //    }
                                ...
                                if (!_lstWorkflowRelated[_positonToAction].ItemID.ToString().Equals(_workflowItem.ID)) // Nếu khác ID Related -> chuyển qua lấy RLID
                                {
                                    _currentWFItem.ID = _lstWorkflowRelated[_positonToAction].ItemID.ToString();
                                    _currentWFItem.ListId = _lstWorkflowRelated[_positonToAction].ListId.ToString();
                                    _currentWFItem.Content = _lstWorkflowRelated[_positonToAction].WorkflowContent;
                                }
                                else // giống -> qua lấy RLID
                                {
                                    _currentWFItem.ID = _lstWorkflowRelated[_positonToAction].ItemRLID.ToString();
                                    _currentWFItem.ListId = _lstWorkflowRelated[_positonToAction].ListRLID.ToString();
                                    _currentWFItem.Content = _lstWorkflowRelated[_positonToAction].WorkflowContentRL;
                                }

                                _currentWFItem.Created = _lstWorkflowRelated[_positonToAction].Created;
        ...
        private void ShowPopup_ControlInputGridDetail_InnerAction(ViewElement clickedElement, int actionID, int positionToAction)
        {
            try
            {
                ...
                TextView _tvClose = scheduleDetail.FindViewById<TextView>(Resource.Id.tv_PopupControl_InputGridDetail_Close);
                TextView _tvDone = scheduleDetail.FindViewById<TextView>(Resource.Id.tv_PopupControl_InputGridDetail_Done);
                ...
                if (CmmVariable.SysConfig.LangCode.Equals(CmmDroidVariable.M_SysLangVN))
                {
                    ...
                    _tvDone.Text = CmmFunction.GetTitle("TEXT_DONE", "Hoàn tất");
                }
                else
                {
                    _tvTitle.Text = clickedElement.Title;
        ...
        /// <summary>
        /// Update giá trị JObect cho popup item dc click vào. => chưa cập nhật vào list chính
        /// </summary>
            ...
            try
            {
                ...
                switch (buttonAction.ID)
                {
                    ...
                    case (int)(WorkflowAction.Action.RequestIdea): // 9 tham vấn ý kiến
                        {
                            SharedView_PopupActionForward _popupActionForward = new SharedView_PopupActionForward(_inflater, _mainAct, this, "FragmentDetailWorkflow", _rootView, popupScaleWidth_Full);
                    ...
                    case (int)(WorkflowAction.Action.RequestInformation):// 7 - Action bổ sung thông tin
                        {
                            ...
                            SharedView_PopupActionRequestInfo _popupActionRequestInfo = new SharedView_PopupActionRequestInfo(_inflater, _mainAct, this, "FragmentDetailWorkflow", _rootView,
                                popupScaleWidth_Full, _lstUserAll);
            ...
            try
            {
                ...
                await Task.Run(() =>
                {
                    ...
                    if (_result)
                    {
                        ...
                        {
                            CmmDroidFunction.HideProcessingDialog();
                            Click_Back_WithRefreshPage(null, null);
                        });
                    }
                    else
                    {
