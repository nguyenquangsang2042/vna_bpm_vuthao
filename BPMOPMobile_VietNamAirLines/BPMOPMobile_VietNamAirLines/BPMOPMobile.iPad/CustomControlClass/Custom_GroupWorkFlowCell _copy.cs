...
{
    class Custom_GroupWorkFlowCell_copy: UITableViewCell
    {
        UICollectionView collection_groupWorkFlow;
        List<BeanWorkflow> lst_workFlow { get; set; }
        UIViewController viewController { get; set; }

        public Custom_GroupWorkFlowCell_copy(NSString cellID, UIViewController _viewController) : base(UITableViewCellStyle.Default, cellID)
        {
            Accessory = UITableViewCellAccessory.None;
            viewController = _viewController;
        }

        public Custom_GroupWorkFlowCell_copy(IntPtr handle) : base(handle)
        {
        }

        public Custom_GroupWorkFlowCell_copy(UITableViewCellStyle style, string reuseIdentifier) : base(style, reuseIdentifier)
        {
        }
        ...
        {
            Custom_GroupWorkFlowCell_copy parentView { get; set; }
            public List<BeanWorkflow> lst_workFlow { get; set; }

            public Collection_Source(Custom_GroupWorkFlowCell_copy _parentview, List<BeanWorkflow> _lst_workFlow)
            {
        ...
        private class CollectionCell : UICollectionViewCell
        {
            ...
            public async void UpdateRow(BeanWorkflow _workFlow, Custom_GroupWorkFlowCell_copy _parentView, NSIndexPath _indexPath)
            {
                try
                {
        ...
        {
            static Custom_GroupWorkFlowCell_copy parentView;
            Collection_Source collect_source;

            #region Constructors
            public CustomFlowLayoutDelegate(Custom_GroupWorkFlowCell_copy _parent, Collection_Source _collect_source)
            {
