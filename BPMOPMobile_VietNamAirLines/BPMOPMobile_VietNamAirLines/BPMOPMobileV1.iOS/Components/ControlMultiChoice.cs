...
{
    public class ControlMultiChoice : ControlBase
    {
        UIViewController parentView { get; set; }
        NSIndexPath indexPath { get; set; }
        ViewElement element { get; set; }

        public ControlMultiChoice(UIViewController _parentView, ViewElement _element, NSIndexPath _indexPath)
        {
        ...
        public override string Value
        {
            ...
            set
            {
                var _lstLookupData = JsonConvert.DeserializeObject<List<BeanLookupData>>(element.DataSource);

                if (_lstLookupData != null && _lstLookupData.Count > 0 && !String.IsNullOrEmpty(element.Value))
                {
                    List<BeanLookupData> _lstValue = JsonConvert.DeserializeObject<List<BeanLookupData>>(element.Value);
                ...
                var data = JsonConvert.SerializeObject(_lstLookupData.FindAll(s => s.IsSelected)).Trim();
                //var data = value.Trim();
                ...
                //string result;
                //if (data.Contains(";#"))
                //    result = data.TrimStart(";#".ToCharArray()).Replace(";#",", ");
                //else
                //    result = data;
