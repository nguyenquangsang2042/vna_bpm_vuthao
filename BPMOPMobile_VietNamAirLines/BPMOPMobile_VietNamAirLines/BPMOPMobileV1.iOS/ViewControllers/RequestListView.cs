...
{
    public partial class RequestListView : UIViewController
    {
        ...
        UIColor previousColorFilter = UIColor.FromRGB(0, 0, 0);// lay gia trị icon ban dau mac dinh
        ...
        public RequestListView(IntPtr handle) : base(handle)
        {
        }
        ...
        public override void ViewDidLoad()
        {
            ...
            //filter
            //LoadDataFilterTodo(); đã call lúc focus toggle
            //ButtonMenuStyleChange();
        ...
        private void ButtonMenuStyleChange()
        {
            ...
            //loadData_count(GetCountNumber());
            string str_daxuly = CmmFunction.GetTitle("TEXT_PROCESSED", "Đã xử lý");
            string str_dangxuly = BT_Inprogress.Title(UIControlState.Normal);
            ...
            if (tab_inprogress)
            {
                ...
                if (str_dangxuly.Contains("("))
                {
                    ...
                    att_dangxl.AddAttribute(UIStringAttributeKey.ForegroundColor, UIColor.FromRGB(255, 122, 58), new NSRange(indexA, att_dangxl.Length - indexA));
        ...
        private void LoadStatusCategory(string key)
        {
            ...
            {
                Console.WriteLine("RequestListView - LoadStatusCategory - Err: " + ex.ToString());
            }
            finally
            {
                conn.Close();
        ...
        #region toMe
        //LOAD DATA
        private void loadData_count(int countNumber, bool isProcessing = true)
        {
            int _dataCount = 0;
            try
            {
                //if (tab_inprogress)
                //{
                //_dataCount = GetCountNumber();
                _dataCount = countNumber;
                ...
                {
                    str_dangxuly = str_dangxuly + " (" + (inProcess_toMe_count < 10 ? "0" : "") + inProcess_toMe_count.ToString() + ")";
                    BT_Inprogress.SetTitle(str_dangxuly, UIControlState.Normal);
                }
                else
                {
        ...
        int GetCountNumber()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(count))
                {
                    ...
                    {
                        if (string.Compare(allCount[0], CmmVariable.KEY_COUNT_ASIGNTOME_INPROCESS) == 0)
                        {
        ...
        private void FilterServer(bool isLoadMoreDataFilter)
        {
            try
            {
                ...
                else// Lan dau filter
                {
                    ...
                    if (lst_appBase_cxl != null && lst_appBase_cxl.Count > 0 && tab_inprogress)
            ...
            _ = Task.Run(() =>
             {
                 ...
                 try
                 {
                     lstObj = new ProviderBase().LoadMoreDataTFromSerVer(
                         tab_inprogress ? CmmVariable.KEY_GET_TOME_INPROCESS : CmmVariable.KEY_GET_TOME_PROCESSED,
        ...
        private string CreateQueryToDo(bool isGetCount)
        {
            ...
            {
                //query_inprogress = "NOTI.Type = 1 AND NOTI.Status = 1";
                if (isGetCount)
                    query = string.Format(@"SELECT Count(*) as count
                                        FROM BeanAppBase AB
                                        WHERE AB.StatusGroup <> 256
                                              AND AB.NotifiedUsers LIKE '%{0}%'
                                              AND (AB.ResourceCategoryId <> 16 OR AB.CreatedBy <> '%{0}%') {1}",
                                       CmmVariable.SysConfig.UserId.ToLower(), str_status);
                else
            ...
            return query;*/
            #endregion
            return CmmIOSFunction.GetQueryStringAppBaseVDT(tab_inprogress);
        }
        /// <summary>
        /// sort section from create
        /// </summary>
        ...
        private void SortListAppBase(string query, bool isOnline)
        {
            if (lst_appBase_cxl != null && lst_appBase_cxl.Count > 0)
            {
                ...
                //string a = JsonConvert.SerializeObject(lstItems);
        ...
        private void SortListSearchAppBase(string query, bool isOnline, List<BeanAppBaseExt> lstSearch)
        {
            ...
            table_content.Source = new Todo_TableSource(dict, this, query);
            table_content.ReloadData();
        }
        ...
        private void CmmEvent_UpdateLangComplete(object sender, CmmEvent.UpdateEventArgs e)
        {
            ...
            {
                Console.WriteLine("RequestListView - CmmEvent_UpdateLangComplete - ERR: " + ex.ToString());
            }
        }
        ...
        private void ToggleMenuStatus()
        {
            ...
            string defaultIdAppstatustoMe = CmmFunction.GetAppSettingValue(tab_inprogress ? CmmVariable.APPSTATUS_TOME_DANGXULY : CmmVariable.APPSTATUS_TOME_DAXULY);
            string[] arrayStatus = defaultIdAppstatustoMe.Split(",");
            custom_AppStatusCategory.ListAppStatus = lst_appStatus_toMe.FindAll(s => arrayStatus.FirstOrDefault(o => string.Compare(o, s.ID.ToString()) == 0) != null);//lst_appStatus_toMe;
            custom_AppStatusCategory.ListAppStatus_selected = ListAppStatus_selected_toMe;
        ...
        private void AppLyFilter()
        {
            ...
            else
            {
                ...
                /*if (tab_inprog ress)// dang xu ly thi status = default
                {
                ...
                FilterServer(false);
                //loadData_count(ListAppStatus_selected_toMe != null && ListAppStatus_selected_toMe.Count > 0 ? ListAppStatus_selected_toMe.Count : 0);
                ButtonMenuStyleChange();
        ...
        {
            MyRequestListView myRequest = (MyRequestListView)Storyboard.InstantiateViewController("MyRequestListView");
        ...
        private class Todo_TableSource : UITableViewSource
        {
            ...
            public Todo_TableSource(Dictionary<string, List<BeanAppBaseExt>> _dict_todo, RequestListView _parentview, string query)
            {
