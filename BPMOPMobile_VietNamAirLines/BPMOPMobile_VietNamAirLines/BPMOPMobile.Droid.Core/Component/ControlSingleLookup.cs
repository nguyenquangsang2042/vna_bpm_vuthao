...
{
    class ControlSingleLookup : ControlBase
    {
            ...
            set
            {
                if (!String.IsNullOrEmpty(_element.Value))
                {
                    ...
                    if (_lstValue != null && _lstValue.Count > 0)
                        this._tvValue.Text = _lstValue[0].Title.ToString();
                    else
                        this._tvValue.Text = "";
                }
                else
        ...
        public ControlSingleLookup(Activity _mainAct, LinearLayout _parentView, ViewElement _element) : base(_mainAct)
        {
        ...
        public ControlSingleLookup(Activity _mainAct, LinearLayout _parentView, ViewElement _elementParent, ViewElement _elementChild, JObject _JObjectChild, int _flagView) : base(_mainAct)
        {
            // Để render theo base
            this._parentView = _parentView;
