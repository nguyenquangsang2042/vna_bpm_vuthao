...
{
    public class ControlMultiSelectUser : ControlBase
    {
        ...
        public override string Value
        {
            set
            {
                ...
                {
                    for (int i = 0; i < _beanUserAndGroup.Count; i++)
                        _lstName.Add(_beanUserAndGroup[i].Name.Trim());

                    _result = String.Join(", ", _lstName.ToArray());
                ...
                else // Hiện Hint lên + chữ nghiêng
                {
                    if (_element.Enable)
                    {
                        if (_element.DataType.Equals("selectusermulti"))
        ...
        public ControlMultiSelectUser(Activity _mainAct, LinearLayout _parentView, ViewElement _element) : base(_mainAct)
        {
        ...
        public ControlMultiSelectUser(Activity _mainAct, LinearLayout _parentView, ViewElement _elementParent, ViewElement _elementChild, JObject _JObjectChild, int _flagView) : base(_mainAct)
        {
            // Để render theo base
            this._parentView = _parentView;
