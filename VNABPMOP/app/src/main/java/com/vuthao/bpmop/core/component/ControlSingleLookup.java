...
public class ControlSingleLookup extends ControlBase {
    ...
    public ControlSingleLookup(Activity mainAct, LinearLayout parentView, ViewElement element) {
        super(mainAct);
        this.element = element;
        initializeComponent();
    }

    public ControlSingleLookup(Activity mainAct, LinearLayout _parentView, ViewElement elementParent, ViewElement elementChild, JSONObject JObjectChild, int flagView) {
        super(mainAct);
        // Để render theo base
    ...
    public void setValue(String value) {
        if (!Functions.isNullOrEmpty(element.getValue())) {
            ...
            ArrayList<LookupData> _lstObject = new Gson().fromJson(data, new TypeToken<ArrayList<LookupData>>() {
            }.getType());

            ArrayList<String> _lstValue = new ArrayList<>();

            if (_lstObject != null && _lstObject.size() > 0) {
                for (LookupData item : _lstObject) {
                    _lstValue.add(item.getTitle());
                }

                result = String.join(", ", _lstValue);
            }
